...
static VOID CmdReStartDLRsp(struct cmd_msg *msg, UINT32 *Data, UINT16 Len)
{
	FW_RXD_2 Status;
		...
		DlListForEachSafe(msg, msg_tmp, &ctl->ackq, struct cmd_msg, list) {
			if (msg->seq == event_rxd->fw_rxd_1.field.seq_num)
			{
				...
				if ((event_rxd->fw_rxd_1.field.eid == MT_FW_START_RSP)
					...
					msg->rsp_handler(msg, (UINT32*)(GET_OS_PKT_DATAPTR(net_pkt) + sizeof(*event_rxd) - 4),
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->WOW_Cfg.nWakeupInterface == WOW_GPIO_LOW_TO_HIGH)
		CmdWakeupOption.GpioParameter |= 0x4;
	else
		CmdWakeupOption.GpioParameter |= 0x2;
	
	DBGPRINT(RT_DEBUG_OFF, ("%s:(WakeupInterface=%d, GPIONumber=%d, GPIOTimer=%d)\n", __FUNCTION__, CmdWakeupOption.WakeupInterface, 
						CmdWakeupOption.GPIONumber, CmdWakeupOption.GPIOTimer));
