...
/*	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);*/
    ...
    if ((pAd->Mlme.PeriodicRound % 10) == 0)
    {
           ...
           )
        {
            ...
            { /* TxopThreshold unit is Mbps */
                if (TxTotalByteCnt > RxTotalByteCnt)
                {
                    ...
                    { /* Boundary unit is Ratio */
                        DBGPRINT(RT_DEBUG_TRACE, ("(Ratio: %d >= %d) Should be enable TxOP to 0x60\n", 
                    ...
                    else if ((TxTotalByteCnt/RxTotalByteCnt) <= pAd->CommonCfg.ManualTxopLowBound)
                    {
                        ...
                        {
                            DBGPRINT(RT_DEBUG_TRACE, ("(Ratio: %d <= %d) Should be disable TxOP to 0x0\n", 
                    ...
                    {
                        DBGPRINT(RT_DEBUG_TRACE, ("(Ratio: %d < %d < %d) Keep the last TxOP setting\n", 
            ...
            {
                DBGPRINT(RT_DEBUG_TRACE, ("Current TP=%d < Threshold(%d), turn-off TxOP\n", 
